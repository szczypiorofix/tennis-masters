...
export const getAllUsers = asyncErrorHandlerMiddleware(
    async (request: Request, response: Response<ServerResponseUsers>) => {
        const usersWithSchema: MongooseDocument<IUser>[] =
            await UserModel.find();
        const users: IUser[] = MongooseDocumentMapper<IUser>(
            usersWithSchema,
            IUserDefaults
        );
...
export const createUser = asyncErrorHandlerMiddleware(
...
export const getUserById = asyncErrorHandlerMiddleware(
...
export const deleteUserById = asyncErrorHandlerMiddleware(
    async (request: Request, response: Response<ServerResponseUser>) => {
        const resp: ServerResponseUser = {
            code: 200,
            error: false,
...
export const updateUserById = asyncErrorHandlerMiddleware(
    async (request: Request, response: Response<ServerResponseUser>) => {
        const resp: ServerResponseUser = {
            code: 200,
            error: false,
